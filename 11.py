...
class BlueBlob(Blob):
    ...
    def __init__(self, x_boundary, y_boundary):
        # We can also use: super().__init__((0, 0, 255), x_boundary, y_boundary)
        Blob.__init__(self, (0, 0, 255), x_boundary, y_boundary)

    # Operator overloading
    def __add__(self, other_blob):
        logging.info(
            "Blob add op {} + {}".format(repr(self), repr(other_blob)))
        if other_blob.color == (255, 0, 0):
            ...
            raise Exception(
                "Tried to combine one or more blobs of unsupported colors")
...
def handle_collisions(blob_list):
    blues, reds, greens = blob_list  # blob list is a list of 3 dicts
    for blue_id, blue_blob in blues.copy().items():
        for other_blobs in reds, greens:
...
if __name__ == '__main__':
    ...
    # green_blobs = dict(enumerate([GreenBlob(WIDTH, HEIGHT)
    #                    for i in range(STARTING_GREEN_BLOBS)]))
    # print(blue_blobs[0].__repr__())
    # print(blue_blobs[0].__str__())
    # pygame.quit()
